...
4.2. IntellisenseCreator.exe
  Usage: IntellisenseCreator.exe output_file script_dll extra_dll
  This tool extracts information on server, client and mapper APIs from the ASCompiler dll files for the Intellisense purpose. It first registers all AngelScript functions and classes provided by the dlls, and then scan the dlls to search the argument names (if present). The results are saved as a header file.
...
4. Run update_intellisense.bat to create the intellisense*.h files.
